...
def save_data(data, option='json'):
    ...
    if option=='json':
        ...
        # with open(f'/dbfs/mnt/datalake/TabNews/contents/json/{now}.json', 'w') as open_file: # mounting point of the datalake
            json.dump(data, open_file, indent=4)

    elif option=='dataframe':
        df = pd.DataFrame(data)
        df.to_parquet('./data/contents/parquet/{}.parquet'.format(now), index=False)
        # df.to_parquet('/dbfs/mnt/datalake/TabNews/contents/parquet/{}.parquet'.format(now), index=False)
